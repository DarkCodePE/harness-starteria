...
function provenanceEntries(sourceRefs: string[], status?: string | null): Array<{ sourceRef: string; kind: FramingProvenance }> { const kind = status === 'user_declared' || status === 'ai_inferred' || status === 'ai_suggested' || status === 'user_confirmed' || status === 'extracted' ? status : 'unknown'; return unique(sourceRefs).map((sourceRef) => ({ sourceRef, kind })); }
