  ...
  async execute(input: PortfolioEntrySessionRunInput): Promise<SessionExecutionResult> {
    ...
    while (nextUserInput !== null) {
      ...
      if (context.clarification_status === 'exploration_offered') {
        const optInTransition = applyExplorationChoice(context, input.guidedExplorationChoice ?? 'reject');
        context = {
          ...context,
          interaction_mode: optInTransition.to_mode,
          clarification_status: optInTransition.to_status,
          user_exploration_choice: input.guidedExplorationChoice === 'accept' ? 'accept' : 'reject',
