...
export class PortfolioEntrySessionService {
  ...
  async claimOwnership(sessionId: string, ownerUserId: string, now = this.now()): Promise<PortfolioEntrySession> {
  ...
  async appendTurn(input: SaveTurnInput): Promise<PortfolioEntryTurn> {
    ...
    assertLifecycleTransition(session, updatedSession.lifecycleStatus);
  ...
  async saveHandoff(input: SaveHandoffInput): Promise<PortfolioEntryHandoffRecord> {
    ...
    const session = await this.requireSession(input.sessionId);
    assertSessionIsActive(session, now);
    assertLifecycleTransition(session, 'HANDOFF_READY');
  ...
  async saveConfirmation(input: SaveConfirmationInput): Promise<PortfolioEntryConfirmation> {
    ...
    assertLifecycleTransition(session, nextLifecycle);
...
function deriveLifecycleFromRuntimeStatus(status: SessionContext['clarification_status']): PortfolioEntrySessionLifecycleStatus {
  if (status === 'ready_for_handoff') return 'HANDOFF_ELIGIBLE';
  if (status === 'ended_with_uncertainty' || status === 'abandoned') return 'ABANDONED';
  if (status === 'exploration_offered' || status === 'guided_exploration' || status === 'in_progress') return 'CLARIFYING';
