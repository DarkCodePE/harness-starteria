...
| Cuántos hay | 8 | ver `doc/product-adr/ADR-INDEX.md` |

`ADR-001` de una serie no tiene nada que ver con `ADR-001` de la otra. Cuando
`/starteria-decision` redacte un ADR de producto, usa la serie de `doc/product-adr/` y no continúa
la numeración de esta serie del harness.
...
de forma, no de fondo: está `superseded`, porque fue reemplazado por `ADR-008`.
