...
## Cambios principales de v0.3 candidata

Esta versión incorpora decisiones surgidas de un test real con una iniciativa aprobada por una autoridad de decisión de negocio:

- Los Portfolio Leads designados comparten responsabilidad sobre el portafolio completo; los Frentes y Retos no se reparten como dominios privados.
- Un Reto activo debe tener al menos una iniciativa que lo aborde; la pérdida de cobertura genera una alerta y una decisión obligatoria.
- Se separa responsabilidad de gobernanza del Portfolio Lead de responsabilidad de ejecución del Initiative Owner.
- Una iniciativa importada o aprobada no queda automáticamente activa en Steps.
- Portfolio Lead mantiene seguimiento continuo de estado, bloqueos, capacidad, cobertura, evidencia, contribución y decisiones.
- El cierre de una iniciativa produce un Brief de Iniciativa dirigido a la autoridad de decisión correspondiente.
...
- La decisión v0.2 que unificaba **Sponsor / Responsable del Reto** queda reemplazada en v0.3: Portfolio Leads gobiernan conjuntamente los Retos y la Autoridad de Decisión de Negocio conserva la autoridad configurada.
...
- ejecutar una decisión reservada a una Autoridad de Decisión de Negocio.
...
- Portfolio Leads responsables de gobernanza y Autoridad de Decisión de Negocio configurada.
...
|---|---|---|---|
| Portfolio Lead | Gobernar conjuntamente el portafolio completo y asegurar que sus iniciativas lleguen a decisión o cierre | Confirmar frentes, estructurar Retos, activar cobertura, asignar Initiative Owners, seguir, desbloquear y preparar decisiones | Alta / recurrente |
| Autoridad de Decisión de Negocio | Ejercer autoridad organizacional sobre una decisión definida | Aprobar metas, capacidad, inversión, continuidad, escala o cierre según alcance y mandato | Por decisión |
...
## 5.1 Responsabilidad compartida de Portfolio Lead

- La organización, mediante Admin o autoridad equivalente, designa uno o más Portfolio Leads.
- Un Portfolio Lead no puede conceder el rol Portfolio Lead a otra persona.
- Todos los Portfolio Leads designados comparten visibilidad y responsabilidad de gobernanza sobre el portafolio completo.
- Crear un Frente o Reto registra procedencia, pero no crea propiedad exclusiva sobre ese objeto.
- No se asigna obligatoriamente un Portfolio Lead específico a cada Reto.
- Los Portfolio Leads aseguran conjuntamente cobertura, seguimiento, desbloqueo y llegada a decisión de las iniciativas.
- La Autoridad de Decisión de Negocio mantiene la autoridad que la organización haya configurado para cada tipo y alcance de decisión.

## 5.2 Portfolio accountability vs. Initiative ownership

El Portfolio Lead es accountable por la gobernanza y finalización defendible de las iniciativas del portafolio. El Initiative Owner es responsable de desarrollar una iniciativa concreta, coordinar al equipo, registrar evidencia y proponer una recomendación.

Una misma persona puede ejercer ambos roles, pero Starteria debe conservarlos como responsabilidades y acciones diferenciadas. El acceso al dashboard de Portfolio no activa por sí mismo un ciclo en Steps.

La autoridad debe asociarse a un tipo de decisión, no suponerse únicamente por acceso al sistema.

## 5.3 Autoridad de Decisión de Negocio

`Business Decision Authority` es una función configurable, no un cargo universal. Puede ser ejercida por una persona o por un grupo, por ejemplo: CEO, fundador, gerente, jefe de departamento, director, sponsor, cliente, comité ejecutivo o comité de innovación.

Una misma iniciativa puede requerir autoridades diferentes según la decisión:

```text
DecisionAuthority
├── personOrGroup
├── organizationRole
├── authorizedDecisionTypes[]
├── scope
├── thresholdsOrConditions
├── approvalMode
└── effectivePeriod
```

Starteria debe identificar quién puede tomar la decisión específica y no inferirlo por el nombre del cargo. La decisión puede ser individual, colegiada o escalonada por monto, riesgo, alcance o impacto.
...
- restricciones;
- Portfolio Leads responsables de gobernanza;
- Autoridad de Decisión de Negocio cuando corresponda;
- decisión esperada;
- estado de cobertura;
- versión actual.
...
Su job operativo permanente es mantener claridad sobre:

- estado y siguiente hito de cada iniciativa;
- bloqueos, alertas y apoyo requerido;
- capacidad comprometida y disponible;
- tiempo restante y condiciones pendientes para resolver cada Reto;
- contribución esperada, observada y atribuida de cada iniciativa;
- brechas de cobertura y nuevas iniciativas candidatas;
- Briefs terminados y decisiones pendientes con la autoridad correspondiente.
...
9. ¿Qué debe reportarse a la Autoridad de Decisión de Negocio correspondiente?
...
Un Reto puede existir sin iniciativas únicamente mientras está en borrador. Para activarse debe tener al menos una iniciativa seleccionada con condiciones mínimas de activación. Si pierde todas sus iniciativas, Starteria marca `coverage_lost` y exige una decisión.

Estados candidatos:

- `draft_without_initiatives`;
- `candidate_coverage`;
- `activation_blocked`;
- `active_coverage`;
- `partial_coverage`;
...
El avance de un Reto no debe reducirse a porcentaje de tareas. Starteria separa, cuando exista información:

- avance de ejecución;
- cobertura;
- evidencia generada;
- cambio observado en la métrica;
- tiempo restante;
- condiciones faltantes para resolver o cerrar.
...
├── scale_similar_area
├── integrate_roadmap
├── seek_decision_authority_alignment
...
- buscar respaldo y Autoridad de Decisión de Negocio;
...
# 37A. Activación y seguimiento Portfolio → Initiative

## 37A.1 Una iniciativa aprobada no está automáticamente activa

Starteria debe distinguir si una iniciativa fue:

- sugerida por IA;
- propuesta por una persona/equipo;
- importada desde una fuente;
- seleccionada por Portfolio;
- aprobada para exploración;
- aprobada para ejecución;
- activada con owner y capacidad.

Antes de abrir un ciclo activo en Steps deben existir o quedar explícitamente pendientes:

```text
Activation Readiness
├── Frente y Reto vinculados
├── contribución esperada
├── alcance del mandato/aprobación
├── decisión que debe habilitar
├── Initiative Owner
├── aceptación del owner
├── capacidad/recursos mínimos
├── restricciones y horizonte
└── estado de activación
```

Estados candidatos de iniciativa:
...
Starteria no debe pasar de Portfolio a Steps silenciosamente. Si la misma persona es Portfolio Lead e Initiative Owner, debe asumir explícitamente el segundo rol y aceptar las condiciones de la iniciativa antes de activar su ciclo.

## 37A.2 Seguimiento continuo

El dashboard de Portfolio Lead consume resúmenes estructurados del Core de Iniciativas sin duplicar la iniciativa. Debe mostrar como mínimo:

- Frente y Reto;
- Initiative Owner y equipo;
- estado, ciclo, Step y siguiente hito;
- fecha objetivo y tiempo restante;
- capacidad utilizada/comprometida cuando exista;
- bloqueos, alertas, severidad y acción requerida;
- contribución esperada, observada y atribuida;
- nivel de evidencia/confianza;
- decisión pendiente;
- nuevas iniciativas candidatas y brechas de cobertura.

Los detalles de ejecución viven en Steps. Portfolio muestra la síntesis necesaria para gobernar, intervenir y reportar.

## 37A.3 Alertas y escalamiento

Una alerta debe registrar:

- situación;
- iniciativa/Reto afectado;
- impacto probable;
- fecha de apertura;
- owner operativo;
- apoyo o decisión requerida;
- responsable del siguiente movimiento;
- fecha límite;
- estado de resolución.

Starteria distingue alertas resolubles por el equipo, por Portfolio Lead, por la Autoridad de Decisión de Negocio o por una dependencia externa.

## 37A.4 Contribución a métricas

Portfolio nunca presenta como equivalente:

```text
contribución esperada ≠ contribución observada ≠ contribución atribuida
```

- **Esperada:** hipótesis sobre lo que la iniciativa podría mover.
- **Observada:** cambio registrado durante o después de la ejecución.
- **Atribuida:** parte del cambio que puede vincularse razonablemente a la iniciativa.

Si falta evidencia, la UI debe mostrar `pending` o `not_evaluable`; no convertir avance operativo en impacto.

## 37A.5 Brief de Iniciativa y decisión de negocio

Una iniciativa termina su ciclo cuando puede entregar un Brief de Iniciativa defendible, no solo cuando completa tareas.

El Brief incluye:

- decisión solicitada;
- Frente, Reto y contribución esperada;
- alcance y trabajo realizado;
- recursos utilizados;
- evidencia y resultados vs. baseline/target;
- aprendizajes y límites de atribución;
- riesgos y dependencias;
- recomendación del equipo;
- alternativas de continuidad.

El Portfolio Lead revisa el Brief y lo dirige a la Autoridad de Decisión de Negocio correspondiente. La decisión registrada puede ser:
...
- pausar;
- cerrar;
- conservar como aprendizaje.

Una iniciativa con evidencia negativa puede estar correctamente terminada. Una iniciativa sin decisión, continuidad o cierre explícito no se considera finalizada.
...
10. Cómo se configura y verifica la Autoridad de Decisión de Negocio por tipo, alcance y umbral de decisión.
...
19. Condiciones mínimas exactas para declarar `ready_to_activate` por tipo de iniciativa.
20. Cómo medir progreso de Reto sin producir un porcentaje opaco o engañoso.
21. Cuándo una alerta debe escalar automáticamente de equipo a Portfolio y de Portfolio a la Autoridad de Decisión de Negocio.
...
# 40. Base utilizada para v0.3 candidata
...
| Sponsor y Challenge Owner se solapan | Unificar experiencia MVP en `Sponsor / Responsable del Reto`, manteniendo autoridad configurable | Reemplazado por decisión v0.3 |
...
# 42. Decisiones incorporadas en v0.3 candidata

| Observación de test real | Decisión v0.3 candidata | Estado |
|---|---|---|
| Portfolio Lead fue llevado a Steps sin aclarar responsabilidad | Prohibir transición silenciosa y exigir Activation Readiness | Incorporado / por re-testear |
| Una iniciativa aprobada fue tratada como ciclo activo | Separar aprobación, selección, ownership, resourcing y activación | Incorporado / por re-testear |
| Se asumió que Portfolio Lead ejecutaría el piloto | Separar Portfolio accountability de Initiative ownership | Incorporado / por re-testear |
| Varios Portfolio Leads podían interpretarse como dueños de partes distintas | Definir responsabilidad conjunta sobre todo el portafolio | Incorporado / por re-testear |
| Un Reto podía quedar estable sin iniciativas | Permitirlo solo como borrador; bloquear activación y alertar pérdida de cobertura | Incorporado / por re-testear |
| El seguimiento se reducía a estado general | Incorporar estado, hitos, bloqueos, capacidad, evidencia, contribución y decisiones | Incorporado / por re-testear |
| El cierre no generaba un artefacto de decisión explícito | Incorporar Brief de Iniciativa + decisión con la autoridad correspondiente | Incorporado / por re-testear |
