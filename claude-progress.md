...
- Verificación ejecutada: ninguna ejecución de tests todavía — este bootstrap solo establece el sistema de registro. `init.sh` aún no se ha corrido en esta sesión.
- Evidencia capturada: estructura de comandos derivada de `front/package.json`; features derivadas de `git log` y `front/src/app/routes.ts`.
- Commits: pendiente (este cambio).
