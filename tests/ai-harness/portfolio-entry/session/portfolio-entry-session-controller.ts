...
import { createScriptedResponderState, respondToQuestions } from './scripted-user-responder';
...
export class PortfolioEntrySessionController {
  ...
  async execute(fixture: MultiTurnFixtureV2): Promise<SessionExecutionResult> {
    let context = createInitialSessionContext(fixture);
    let nextUserInput: string | null = fixture.initial_user_message;
    ...
    while (nextUserInput !== null) {
      ...
      const output = await this.adapter.analyzeTurn({
        entryId: `${fixture.case_id}-${this.options.runId}-${turnIndex}`,
        sessionId: `${fixture.case_id}-${this.options.runId}`,
      ...
      if (budgetApplication.emitted_questions.length > 0 && !terminalStatuses.has(context.clarification_status)) {
        scriptedResponseResult = respondToQuestions(budgetApplication.emitted_questions, fixture.response_rules, responderState);
        if (scriptedResponseResult.response) {
          nextUserInput = scriptedResponseResult.response;
        } else {
          context = {
            ...context,
            clarification_status: 'ended_with_uncertainty',
      ...
      if (context.clarification_status === 'exploration_offered') {
        const optInTransition = applyExplorationChoice(context, fixture);
        context = {
          ...context,
          interaction_mode: optInTransition.to_mode,
          clarification_status: optInTransition.to_status,
          user_exploration_choice: fixture.session.exploration_policy === 'accept_if_offered' ? 'accept' : 'reject',
          exploration_round: optInTransition.to_status === 'guided_exploration' ? context.exploration_round + 1 : context.exploration_round,
          questions_asked_current_round: 0,
          exploration_goal: optInTransition.to_status === 'guided_exploration' ? 'continue_clarifying_structured_question_plan' : context.exploration_goal,
          stop_reason: terminalStatuses.has(optInTransition.to_status) ? optInTransition.reason : null,
        };
...
function applyExplorationChoice(context: SessionContext, fixture: MultiTurnFixtureV2): SessionTransition {
  if (fixture.session.exploration_policy === 'accept_if_offered') {
