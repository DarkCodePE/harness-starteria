...
import type { PortfolioEntryHarnessProviderConfig } from '../model/provider-config';
...
export class FetchStructuredModelAdapter implements StructuredModelAdapter {
  constructor(
    private readonly config: PortfolioEntryHarnessProviderConfig,
  ...
  async generate<T>(input: StructuredModelGenerateInput<T>): Promise<ModelExecutionResult<T>> {
    ...
    while (true) {
      ...
      try {
        ...
        if (!parsed.success) {
          return this.result(input, providerRaw, null, null, [parsed.error], duration, attempt - 1, retryReason, 'SCHEMA_ERROR');
        }
